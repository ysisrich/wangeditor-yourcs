...
import { IDomEditor, DomEditor } from "@wangeditor3/core-yourcs"
    ...
    if (selectedNode) {
      ...
      if (next) {
        if (next[0] && (next[0] as BaseText).text) {
          ...
          next =
            (Editor.above(editor, { at: next[1] }) as NodeEntry<Descendant>) ??
            next
        }
        ...
        if (
          DomEditor.checkNodeType(
            topLevelNodes[topLevelNodesLength - 1],
            "table"
          )
        ) {
