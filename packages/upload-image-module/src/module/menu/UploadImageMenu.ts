...
import { IButtonMenu, IDomEditor, t } from "@wangeditor3/core-yourcs"
import {
  insertImageNode,
  isInsertImageMenuDisabled
} from "@wangeditor3/basic-modules-yourcs"
...
class UploadImage implements IButtonMenu {
  ...
  exec(editor: IDomEditor, value: string | boolean) {
    const { allowedFileTypes = [], customBrowseAndUpload } =
      this.getMenuConfig(editor)

    // 自定义选择图片，并上传，如图床
    if (customBrowseAndUpload) {
      customBrowseAndUpload((src, alt, href) =>
